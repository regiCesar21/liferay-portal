...
public class FragmentEntryLinkPersistenceImpl
	 ...
	 *
	 * @param companyId the company ID
	 * @param rendererKeies the renderer keies
	 * @return the matching fragment entry links
	 */
	@Override
	public List<FragmentEntryLink> findByC_R(
		long companyId, String[] rendererKeies) {

		return findByC_R(
			companyId, rendererKeies, QueryUtil.ALL_POS, QueryUtil.ALL_POS,
			null);
	}
	...
	@Override
	public List<FragmentEntryLink> findByC_R(
		long companyId, String[] rendererKeies, int start, int end) {

		return findByC_R(companyId, rendererKeies, start, end, null);
	}
	...
	public List<FragmentEntryLink> findByC_R(
		...
		return findByC_R(
			companyId, rendererKeies, start, end, orderByComparator, true);
	}
	...
	public List<FragmentEntryLink> findByC_R(
		...
		if (rendererKeies == null) {
			rendererKeies = new String[0];
		}
		else if (rendererKeies.length > 1) {
			for (int i = 0; i < rendererKeies.length; i++) {
				rendererKeies[i] = Objects.toString(rendererKeies[i], "");
			}

			rendererKeies = ArrayUtil.sortedUnique(rendererKeies);
		}

		if (rendererKeies.length == 1) {
			return findByC_R(
				companyId, rendererKeies[0], start, end, orderByComparator);
		}
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				finderArgs = new Object[] {
					companyId, StringUtil.merge(rendererKeies)
				};
			}
		}
		else if (useFinderCache) {
			finderArgs = new Object[] {
				companyId, StringUtil.merge(rendererKeies), start, end,
				orderByComparator
			};
		}
		...
		if (list == null) {
			...
			if (rendererKeies.length > 0) {
				sb.append("(");

				for (int i = 0; i < rendererKeies.length; i++) {
					String rendererKey = rendererKeies[i];
			...
			try {
				...
				for (String rendererKey : rendererKeies) {
	 ...
	 */
	@Override
	public int countByC_R(long companyId, String[] rendererKeies) {
		if (rendererKeies == null) {
			rendererKeies = new String[0];
		}
		else if (rendererKeies.length > 1) {
		...
		if (count == null) {
			...
			try {
				...
				finderCache.removeResult(
					_finderPathWithPaginationCountByC_R, finderArgs);

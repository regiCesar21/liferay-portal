...
if (!gadgets.rpc) {
	// make lib resilient to double-inclusion
	...
	gadgets.rpc = (function() {
		...
		function setupFrame(frameId, token, forcesecure) {
			...
			if (
				setup[frameId] !== true &&
				setup[frameId]++ < SETUP_FRAME_MAX_TRIES
			) {
		...
		function setupContainerGadgetContext(rpctoken, opt_forcesecure) {
			...
			function init(config) {
				...
				if (
					...
					if (
						...
						if (parentRelayUrl.substring(0, 1) !== '/') {
							...
							parentRelayUrl =
								params.parent.substring(0, lastSlash + 1) +
								parentRelayUrl;
						} else {
							// Host-relative.
							parentRelayUrl =
				...
				var forceSecure =
					opt_forcesecure || params.forcesecure || false;
		...
		function setupReceiver(
			...
			if (targetId === '..') {
				...
				var rpctoken =
					opt_authtoken || params.rpctoken || params.ifpctok || '';
		...
		return /** @scope gadgets.rpc */ {
			...
			register: function(serviceName, handler) {
				...
				if (serviceName === DEFAULT_NAME) {
					throw new Error(
						'Cannot overwrite default service:' +
							' use registerDefault'
					);
				}
			...
			call: function(targetId, serviceName, callback, var_args) {
				...
				if (targetId !== '..' && !document.getElementById(targetId)) {
					...
					gadgets.log(
						'WARNING: attempted send to nonexistent frame: ' +
							targetId
					);
					return;
				}

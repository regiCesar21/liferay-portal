...
public class CommerceAccountPersistenceImpl
	...
	public List<CommerceAccount> filterFindByCompanyId(
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sb.append(
					CommerceAccountModelImpl.ORDER_BY_SQL_INLINE_DISTINCT);
			}
			else {
	...
	public CommerceAccount updateImpl(CommerceAccount commerceAccount) {
		...
		if (Validator.isNull(commerceAccount.getExternalReferenceCode())) {
			...
			if (!Objects.equals(
				...
				if (userId > 0) {
					long companyId = commerceAccount.getCompanyId();
					...
					if (!isNew) {
						classPK = commerceAccount.getPrimaryKey();
					}
					...
					try {
						commerceAccount.setExternalReferenceCode(
							SanitizerUtil.sanitize(
								...
								CommerceAccount.class.getName(), classPK,

...
import com.liferay.portal.upgrade.internal.release.helper.ReleaseManagerHelper;
...
public class ReleaseManagerOSGiCommands {
	...
	public String execute(String bundleSymbolicName) {
		List<UpgradeInfo> upgradeInfos = _releaseManagerHelper.getUpgradeInfos(
			bundleSymbolicName);
		...
		TeeLoggingUtil.runWithTeeLogging(
			() -> _upgradeExecutor.executeUpgradeInfos(
				...
				releaseGraphManager.getUpgradeInfos(
					_releaseManagerHelper.getSchemaVersionString(
						bundleSymbolicName),
					toVersionString)));
	...
	public String list() {
		Set<String> bundleSymbolicNames =
			_releaseManagerHelper.getBundleSymbolicNames();
	...
	public String list(String bundleSymbolicName) {
		...
		sb.append(
			_releaseManagerHelper.getSchemaVersionString(bundleSymbolicName));
		sb.append(StringPool.NEW_LINE);
	...
	protected void executeAll(
		...
		while (true) {
			Set<String> upgradableBundleSymbolicNames =
				_releaseManagerHelper.getUpgradableBundleSymbolicNames();
			...
			for (String upgradableBundleSymbolicName :
				...
				try {
					List<UpgradeInfo> upgradeInfos =
						_releaseManagerHelper.getUpgradeInfos(
	...
	@Reference
	private ReleaseManagerHelper _releaseManagerHelper;

...
public class DatabaseTestUtil {
			...
			try (ResultSet resultSet = databaseMetaData.getTables(
				...
				while (resultSet.next()) {
					...
					indexColumnNames.addAll(
						_getIndexColumnNames(connection, db, tableName));
				}
			}
		}
	...
	public static void importSQL(String sql, DataSource targetDataSource)
		...
		try (Connection connection = targetDataSource.getConnection()) {
			DB currentDB = DBManagerUtil.getDB();

			DB db = DBManagerUtil.getDB(
				currentDB.getDBType(), targetDataSource);

			db.runSQLTemplateString(connection, sql, true, true);
		}
	}
		...
		try (ResultSet resultSet = db.getIndexResultSet(
			...
			while (resultSet.next()) {
				indexColumnNames.add(
					StringUtil.merge(
						new Object[] {
							tableName, resultSet.getString("INDEX_NAME"),
							resultSet.getString("NON_UNIQUE"),
							resultSet.getString("COLUMN_NAME"),
